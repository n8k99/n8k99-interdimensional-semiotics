---
title: "Confirmation Bias"
type: "[[Concept]]"
domain: "[[The Commons]]"
Lifestage: "🌱 Seed"
tags:
  - interdimensional-semiotics
  - concept
  - software-law
  - decisions
aliases: []
attribution: "Cognitive psychology"
category: Decisions
source: "https://lawsofsoftwareengineering.com/"
---

# Confirmation Bias

> The tendency to search for, interpret, and recall information that confirms one's existing beliefs.

**Attribution:** Cognitive psychology
**Category:** Decisions

## Statement

Active in code review, debugging ("it can't be that, I just checked it"), architecture arguments, and hiring. The countermeasure is structural — checklists, red teams, forcing disconfirmation — not willpower.

## Significance for Interdimensional Semiotics

> *How does this law function as a sign operating across substrates? Which [[HEMM Space]] layer does it inhabit — is it a property of the symbolic order, the material substrate, or the meaning-making process itself?*

## Cross-Domain Resonance

> *Where else does this pattern appear outside of software — in writing, music composition, biology, organizational dynamics, [[T.A.S.K.S.]] observation architecture?*

## Related Laws

- [[Dunning-Kruger Effect]]
- [[Hanlon's Razor]]
- [[Occam's Razor]]
- [[Sunk Cost Fallacy]]
- [[Map Is Not the Territory]]
- [[Amara's Law]]

## Source

- [Laws of Software Engineering](https://lawsofsoftwareengineering.com/) — Dr. Milan Milanović
