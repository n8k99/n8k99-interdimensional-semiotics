...
**Transcendentals** — positions not reachable by any algebraic construction over the integers. A CIF involving $\pi$, $e$, or $e^{\sqrt{2}}$ addresses a cosmos that *cannot be constructed* from prime-space using any finite algebraic procedure. Transcendental-positioned cosmoi are reachable only by limit processes, recursive iteration, or [[Homotopy|homotopic]] continuation. They are the cosmoi that no engineering process operating from inside an algebraic cosmos can build deliberately. They can be *navigated to*; they cannot be *constructed*.
...
Iterative maps of this kind are the natural notation for CIF because they remain structurally consistent through the cosmological discontinuities they describe. The map is its own [[Topological Invariant|topological invariant]]. This is why CIFs that look like algebraic expressions ($17\pi^i$, $e^{\sqrt{2}}$) are best understood as *seed values* for iterations, not as arithmetic results: the meaningful object is the orbit of the iteration starting from that seed, not the seed itself.
